...
class ProposalTargetLayer(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        Returns:
            batch_dict:
                ...
                rcnn_cls_labels: (B, M)
        """
        batch_rois, batch_gt_of_rois, batch_roi_ious, batch_roi_scores, batch_roi_labels, batch_gt_assignments = \
        ...
        # my code
        re_weight_rcnn_cls_labels = None
        if 'select_low_idx' in batch_dict.keys():
            re_weight_rcnn_cls_labels = torch.ones(batch_cls_labels.size(), device=batch_cls_labels.device)
            for bs_idx in range(batch_dict['batch_size']):
                low_idx = batch_dict['select_low_idx'][bs_idx]
                score = batch_dict['select_low_score'][bs_idx]
                cur_gt_assignment = batch_gt_assignments[bs_idx]
                if len(low_idx) > 0:
                    for cur_idx, cur_score in zip(low_idx, score):
                        re_weight_idx = cur_gt_assignment == cur_idx
                        re_weight_rcnn_cls_labels[bs_idx][re_weight_idx] = \
                            re_weight_rcnn_cls_labels[bs_idx][re_weight_idx] * (cur_score / 2 * 2)

        targets_dict = {'rois': batch_rois, 'gt_of_rois': batch_gt_of_rois, 'gt_iou_of_rois': batch_roi_ious,
                        'roi_scores': batch_roi_scores, 'roi_labels': batch_roi_labels,
                        'reg_valid_mask': reg_valid_mask,
                        'rcnn_cls_labels': batch_cls_labels,
                        're_weight_rcnn_cls_labels': re_weight_rcnn_cls_labels}
    ...
    def sample_rois_for_rcnn(self, batch_dict):
        ...
        batch_gt_assignment = rois.new_zeros(batch_size, self.roi_sampler_cfg.ROI_PER_IMAGE)
        ...
        for index in range(batch_size):
            ...
            batch_gt_assignment[index] = gt_assignment[sampled_inds]

        return batch_rois, batch_gt_of_rois, batch_roi_ious, batch_roi_scores, batch_roi_labels, batch_gt_assignment
