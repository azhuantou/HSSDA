...
class VoxelRCNNHead(RoIHeadTemplate):
    ...
    def forward(self, batch_dict):
        ...
        if 'ema_training_flag' in batch_dict.keys():
            training_flag &= batch_dict['ema_training_flag']

        targets_dict = self.proposal_layer(
            batch_dict, nms_config=self.model_cfg.NMS_CONFIG['TRAIN' if (self.training & training_flag) else 'TEST']
        )
        ...
        if not (self.training & training_flag):
