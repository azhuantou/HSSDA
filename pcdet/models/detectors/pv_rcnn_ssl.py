    ...
    def forward(self, batch_dict):
        if 'pseudo_label_flag' in batch_dict.keys():
            ...
            with torch.no_grad():
                ...
                pred_dicts, _ = self.pv_rcnn_ema.post_processing(batch_dict, no_recall_dict=True)
                pred_dicts_no_nms, _ = self.pv_rcnn_ema.post_processing(
                ...
                if dual_threshold is not None:
                    ...
                    pred_dicts_aug, _ = self.pv_rcnn_ema.post_processing(batch_dict_aug, no_recall_dict=True)
            ...
            batch_dict_train_temp = copy.deepcopy(batch_dict_train)
            keep_key = ['points', 'voxels', 'voxel_coords', 'voxel_num_points']
            ...
            for cur_module in self.pv_rcnn.module_list:
                ...
                if 'VoxelBackBone8x' == cur_module.model_cfg['NAME']:
                    ...
                    for key in keep_key:
                        batch_dict_train[key] = batch_dict_train_temp[key]
...
class TrainDataProcessor:
    def __init__(self, root_path):
        ...
        # data_cfg_root = '/data/cqupt/code/semi_idea/tools/cfgs/dataset_configs/kitti_dataset.yaml'
        # pvrcnn_data_cfg_root = '/data/cqupt/code/semi_idea/tools/cfgs/kitti_models/pv_rcnn_ssl.yaml'

        data_cfg_root = root_path / 'tools' / 'cfgs' / 'dataset_configs' / 'kitti_dataset.yaml'
        pvrcnn_data_cfg_root = root_path / 'tools' / 'cfgs' / 'kitti_models' / 'pv_rcnn_ssl.yaml'
    ...
    def get_removal_points_scene_no_label(pred_dicts, batch_dict, label_mask):
        ...
        for index in range(len(batch_dict['gt_boxes'])):
            ...
            if label_mask[index]:  # fully labeled scenes do not change
